...
def estimate_thetas(genotype_dat, tax_list, ind_map, intervals, folded, output_dir):
    ...
    for pop_index, pop in enumerate(populations.keys()):
        ...
        if folded:
            n_chromosomes = count_chromosomes(ind_list, ind_map) // 2 + 1
        else:
            n_chromosomes = count_chromosomes(ind_list, ind_map) + 1
        ...
        S = np.sum(sfs[pop_index, 1:n_chromosomes]) # Number of segregating sites is the sum of the SFS from 1 to n-1
        n_sites = np.sum(sfs[pop_index, :]) # Total number of sites is the sum of the SFS from 0 to n
        ...
        Vd = e1 * S + e2 * S * (S - 1)
        #----------------#
        theta_pi = 1/(combination(n_chromosomes, 2)) * np.sum(np.arange(1, n_chromosomes) * (n_chromosomes - np.arange(1, n_chromosomes)) * sfs[pop_index, 1:n_chromosomes])
        ...
        theta_results.append({'population': pop, 'n_individuals': n_individuals, 'n_chromosomes': n_chromosomes - 1, 'theta_wattersons': theta_wattersons_persite, 'theta_pi': theta_pi_persite, 'tajima_D': tajima_D})
